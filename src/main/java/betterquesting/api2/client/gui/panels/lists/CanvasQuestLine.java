    ...
    public void setQuestLine(IQuestLine line) {
        ...
        for (DBEntry<IQuestLineEntry> qle : line.getEntries()) {
            ...
            GuiRectangle rect = new GuiRectangle(
                    qle.getValue().getPosX(),
                    qle.getValue().getPosY(),
                    qle.getValue().getSizeX(),
        ...
        for (Entry<Integer, PanelButtonQuest> entry : questBtns.entrySet()) {
            ...
            if (QuestingAPI.getAPI(ApiReference.SETTINGS).canUserEdit(player)) {
                List<DBEntry<IQuest>> visReqList = QuestingAPI.getAPI(ApiReference.QUEST_DB)
                        .bulkLookup(quest.getValue().getVisRequirements());
                if (reqList.size() <= 0) continue;
                for (DBEntry<IQuest> visReq : visReqList) {
                    PanelButtonQuest parBtn = questBtns.get(visReq.getID());

                    if (parBtn != null) {
                        PanelLine prLine = new PanelLine(
                                parBtn.getTransform(),
                                entry.getValue().getTransform(),
                                PresetLine.QUEST_LOCKED.getLine(),
                                10,
                                PresetColor.QUEST_LINE_VIS.getColor(),
                                1);

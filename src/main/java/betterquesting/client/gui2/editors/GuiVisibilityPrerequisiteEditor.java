...
public class GuiVisibilityPrerequisiteEditor extends GuiScreenCanvas
        implements IPEventListener, IVolatileScreen, INeedsRefresh {
    ...
    public GuiVisibilityPrerequisiteEditor(GuiScreen parent, IQuest quest) {
        super(parent);
        this.quest = quest;
    ...
    public void initPanel() {
        ...
        PanelTextBox panTxt = new PanelTextBox(
                        ...
                        QuestTranslation.translate("betterquesting.title.vis_pre_requisites"))
                .setAlignment(1);
        ...
        PanelTextBox txtDb = new PanelTextBox(
                        ...
                        QuestTranslation.translate("betterquesting.gui.database"))
                .setAlignment(1)
        ...
        canvasDB = new CanvasQuestDatabase(new GuiTransform(GuiAlign.FULL_BOX, new GuiPadding(0, 32, 8, 24), 0)) {
            ...
            protected boolean addResult(DBEntry<IQuest> entry, int index, int width) {
                PanelButtonStorage<DBEntry<IQuest>> btnAdd =
                        new PanelButtonStorage<>(new GuiRectangle(0, index * 16, 16, 16, 0), 2, "", entry);
                btnAdd.setIcon(PresetIcon.ICON_POSITIVE.getTexture());
                btnAdd.setActive(!containsVisReq(quest, entry.getID()));
                this.addPanel(btnAdd);

                PanelButtonStorage<DBEntry<IQuest>> btnEdit = new PanelButtonStorage<>(
                        new GuiRectangle(16, index * 16, width - 32, 16, 0),
                        1,
                        QuestTranslation.translate(entry.getValue().getProperty(NativeProps.NAME)),
                        entry);
        ...
        PanelVScrollBar scReq =
                new PanelVScrollBar(new GuiTransform(GuiAlign.RIGHT_EDGE, new GuiPadding(-8, 16, 0, 0), 0));
    ...
    private void refreshReqCanvas() {
        ...
        List<DBEntry<IQuest>> arrReq = QuestDatabase.INSTANCE.bulkLookup(quest.getVisRequirements());
        for (int i = 0; i < arrReq.size(); i++) {
            PanelButtonStorage<DBEntry<IQuest>> btnEdit = new PanelButtonStorage<>(
                    new GuiRectangle(0, i * 16, width - 16, 16, 0),
                    1,
                    QuestTranslation.translate(arrReq.get(i).getValue().getProperty(NativeProps.NAME)),
                    arrReq.get(i));
            canvasPreReq.addPanel(btnEdit);

            PanelButtonStorage<DBEntry<IQuest>> btnRem =
    ...
    private void removeVisReq(IQuest quest, int id) {
        ...
        if (hasRemoved) quest.setVisRequirements(rem);
    }

    private void addVisReq(IQuest quest, int id) {
        if (containsVisReq(quest, id)) return;
        int[] orig = quest.getVisRequirements();

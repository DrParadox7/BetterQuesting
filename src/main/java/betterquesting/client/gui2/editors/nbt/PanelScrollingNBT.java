...
public class PanelScrollingNBT extends CanvasScrolling implements IPEventListener {
    ...
    private void refreshList() {
        ...
        if (nbt.getId() == 10) // NBTTagCompound
        {
            ...
            while (keys.hasNext()) {
                ...
                {
                    PanelButtonStorage<String> btn = new PanelButtonStorage<>(
                            new GuiRectangle(lw, i * 16, rw - 32, 16, 0), btnEdit, "List...", k);
                    this.addPanel(btn);
                ...
                } else if (entry.getId() > 1 && entry.getId() < 7) // Number
                {
                    ...
                    PanelTextField<String> text = new PanelTextField<>(
                            new GuiRectangle(lw, i * 16, rw - 48, 16, 0),
                            "int_array" + ((NBTTagIntArray) entry).toString(),
                            FieldFilterString.INSTANCE);
                    text.setMaxLength(Integer.MAX_VALUE);
                    this.addPanel(text);
                    ...
                    PanelTextBox err = new PanelTextBox(
                                    ...
                                    entry.getClass().getSimpleName() + ":" + entry.getId() + " Not Supported Yet")
                            .setAlignment(1);
            ...
            this.addPanel(new PanelGeneric(
                    new GuiRectangle(0, i * 16, width - 32, 16, 0), null)); // Keeps the list from auto resizing
        ...
        } else if (nbt.getId() == 9) // NBTTagList
        {
            ...
            for (; i < tagList.size(); i++) {
                ...
                PanelTextBox name =
                        new PanelTextBox(new GuiRectangle(0, i * 16 + 4, lw - 8, 16, 0), "#" + i).setAlignment(2);
                name.setColor(PresetColor.TEXT_MAIN.getColor());
                this.addPanel(name);
                ...
                {
                    PanelButtonStorage<Integer> btn = new PanelButtonStorage<>(
                            new GuiRectangle(lw, i * 16, rw - 32, 16, 0), btnEdit, "List...", i);
                    this.addPanel(btn);
                } else if (entry.getId() == 8) // Text
                {
                    PanelTextField<String> text = new PanelTextField<>(
    ...
    private void onButtonPress(PEventButton event) {
        ...
        if (!(btn.getButtonID() == btnEdit
                || btn.getButtonID() == btnAdv
                ...
                if (nbt.getId() == 10) {
                    mc.displayGuiScreen(new GuiTextEditor(
                            ...
                            new CallbackNBTTagString(
                                    (NBTTagCompound) nbt, ((PanelButtonStorage<String>) btn).getStoredValue())));
                } else if (nbt.getId() == 9) {
                    mc.displayGuiScreen(new GuiTextEditor(
                            mc.currentScreen,
                            ((NBTTagString) entry).func_150285_a_(),
                            new CallbackNBTTagString(
                                    (NBTTagList) nbt, ((PanelButtonStorage<Integer>) btn).getStoredValue())));
                }
        ...
        } else if (btn.getButtonID() == btnAdv && entry != null) // Open advanced editor (on supported types)
        {
            ...
            if (nbt.getId() == 10) {
                ...
                mc.displayGuiScreen(new GuiNbtAdd(
                        mc.currentScreen, (NBTTagList) nbt, ((PanelButtonStorage<Integer>) btn).getStoredValue()));
            }
    ...
    private String getButtonTitle(NBTTagCompound tag) {
        if (JsonHelper.isItem(tag)) {
            ...
            return QuestTranslation.translate("betterquesting.btn.item") + ": "
                    + (stack == null ? "NULL" : stack.getBaseStack().getDisplayName());
        } else if (JsonHelper.isFluid(tag)) {

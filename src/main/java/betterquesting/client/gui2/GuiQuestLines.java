...
public class GuiQuestLines extends GuiScreenCanvas implements IPEventListener, INeedsRefresh {
    ...
    public void initPanel() {
        ...
        if (canEdit) {
            PanelButton btnEdit = new PanelButton(new GuiTransform(GuiAlign.BOTTOM_LEFT, 8, -56, 16, 16, 0), -1, "")
            ...
            PanelButton btnDesign = new PanelButton(new GuiTransform(GuiAlign.BOTTOM_LEFT, 24, -56, 16, 16, 0), -1, "")
        ...
        cvDescTray.setOpenAction(() -> {
            ...
            if (selectedLine != null) {
                txDesc = new PanelTextBox(
                        new GuiRectangle(0, 0, cvDesc.getTransform().getWidth(), 0, 0),
                        QuestTranslation.translate(selectedLine.getUnlocalisedDescription()),
                        true);
        ...
        btnTrayToggle.setIcon(
                PresetIcon.ICON_BOOKMARK.getTexture(),
                selectedLineId < 0 && !chapterTrayOpened
        ...
        btnTrayToggle.setTooltip(
                Collections.singletonList(QuestTranslation.translate("betterquesting.title.quest_lines")));
        ...
        btnDescToggle.setTooltip(
                Collections.singletonList(QuestTranslation.translate("betterquesting.gui.description")));
        ...
        btnTrayLock.setClickAction((b) -> {
            ...
            ConfigHandler.config
                    .get(Configuration.CATEGORY_GENERAL, "Lock tray", false)
                    .set(trayLock);
            ConfigHandler.config.save();
            ConfigHandler.initConfigs();
        });
        ...
        if (selectedLine != null) {
            ...
            icoChapter.setTexture(
                    new OreDictTexture(1F, selectedLine.getProperty(NativeProps.ICON), false, true), null);
        }
    ...
    private void claimAll() {
        ...
        for (PanelButtonQuest pbQuest : cvQuest.getQuestButtons()) {
            ...
            if (q.getRewards().size() > 0 && q.canClaim(mc.thePlayer))
    ...
    private void buildChapterList() {
        ...
        for (int n = 0; n < visChapters.size(); n++) {
            ...
            cvLines.addPanel(new PanelGeneric(
                    new GuiRectangle(0, n * 16, 16, 16, 0),
                    new OreDictTexture(1F, entry.getValue().getProperty(NativeProps.ICON), false, true)));
            ...
            PanelButtonStorage<DBEntry<IQuestLine>> btnLine = new PanelButtonStorage<>(
                    new GuiRectangle(16, n * 16, listW - 16, 16, 0),
                    1,
                    QuestTranslation.translate(entry.getValue().getUnlocalisedName()),
                    entry);
    ...
    private void openQuestLine(DBEntry<IQuestLine> q) {
        ...
        for (int i = 0; i < btnListRef.size(); i++) {
            btnListRef
                    .get(i)
                    .setActive((visChapters.get(i).getSecond() & 4) == 0
                            && btnListRef.get(i).getStoredValue().getID() != selectedLineId);
        }
    ...
    private void refreshClaimAll() {
        ...
        for (PanelButtonQuest btn : cvQuest.getQuestButtons()) {
            if (btn.getStoredValue().getValue().canClaim(mc.thePlayer)) {
                ...
                claimAll.setIcon(
                        PresetIcon.ICON_CHEST_ALL.getTexture(), new GuiColorPulse(0xFFFFFFFF, 0xFF444444, 2F, 0F), 0);
                return;
            }
        }
    ...
    private void openSearch(PanelButton panelButton) {
        ...
        guiQuestSearch.setCallback(entry -> {
            ...
            Optional<PanelButtonQuest> targetQuestButton = cvQuest.getQuestButtons().stream()
                    .filter(panelButtonQuest ->
                            panelButtonQuest.getStoredValue().getID() == selectedQuestId)
                    .findFirst();
            targetQuestButton.ifPresent(panelButtonQuest -> {
                GuiTextureColored newTexture = new GuiTextureColored(
                        panelButtonQuest.txFrame,
                        new GuiColorPulse(
                                new GuiColorStatic(255, 220, 115, 255), new GuiColorStatic(255, 191, 0, 255), 1, 0));

...
public class QuestInstance implements IQuest
{
    ...
    public boolean canClaimBasically(EntityPlayer player) {
        if (DimensionManager.getProvider(player.dimension).getClass().getName().toLowerCase() != null && DimensionManager.getProvider(player.dimension).getClass().getName().toLowerCase().contains("dreamworld")) return false;
